...
import a103.dev.eyakrev1.databinding.ActivityMainBinding
import a103.dev.eyakrev1.databinding.AlarmTabMainBinding
...
class AlarmFragment : Fragment() {
    ...
    var medicineAlarmList = arrayListOf<a103.dev.eyakrev1.MedicineAlarm>()

    // https://curryyou.tistory.com/386
    // 1. Context를 할당할 변수를 프로퍼티로 선언(어디서든 사용할 수 있게)
    lateinit var mainActivity: a103.dev.eyakrev1.MainActivity
    ...
    override fun onCreateView(
        ...
        for (i in 1..10) {
            // 임시 데이터 넣기
            val medicineAlarm = a103.dev.eyakrev1.MedicineAlarm(
                medicineIcon = "ic_packagepill_afterglow",
                medicineTime = "오후 01:30",
                medicineName = "약이름 $i"
            )
            medicineAlarmList.add(medicineAlarm)
        }

        val alarmListAdapter =
            a103.dev.eyakrev1.AlarmListAdapter(mainActivity, medicineAlarmList)
    ...
    override fun onAttach(context: Context) {
        ...
        mainActivity = context as a103.dev.eyakrev1.MainActivity
    }
}