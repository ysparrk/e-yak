...
import a103.dev.eyakrev1.databinding.ActivityEditMemberBinding
...
//                                Log.d("log", response.body().toString())
    ...
    private fun editCommitButtonClicked(wakeTimeH: String, wakeTimeM: String, breakfastTimeH: String, breakfastTimeM: String,
        ...
        val memberId = pref.getInt("SERVER_USER_ID", -1)
        val serverAccessToken = pref.getString("SERVER_ACCESS_TOKEN", "")
        val data = a103.dev.eyakrev1.ChangeAccountInfoBodyModel(
        ...
        api.changeAccountInfo(memberId = memberId, params = data, Authorization = "Bearer ${serverAccessToken}").enqueue(object: Callback<a103.dev.eyakrev1.ChangeAccountInfoResponseModel> {
            override fun onResponse(call: Call<a103.dev.eyakrev1.ChangeAccountInfoResponseModel>, response: Response<a103.dev.eyakrev1.ChangeAccountInfoResponseModel>) {
                Log.d("log", response.toString())
                Log.d("log", response.body().toString())
            ...
            override fun onFailure(call: Call<a103.dev.eyakrev1.ChangeAccountInfoResponseModel>, t: Throwable) {
                Toast.makeText(applicationContext, "연결이 불안정합니다", Toast.LENGTH_SHORT).show()
            }
        })
