...
import a103.dev.eyakrev1.databinding.ActivityLoginBinding
...
//                    Snackbar.make(binding.root, msg, Snackbar.LENGTH_INDEFINITE).show()
    ...
    private fun tryLoginToServer(loadedGoogleToken: String?, isAutoLogin: Boolean) {
        if (loadedGoogleToken != null) {
            // 서버에 로그인 시도
            val data = a103.dev.eyakrev1.LoginBodyModel("google", loadedGoogleToken)
            api.signIn(data).enqueue(object: Callback<a103.dev.eyakrev1.LoginResponseModel> {
                override fun onResponse(call: Call<a103.dev.eyakrev1.LoginResponseModel>, response: Response<a103.dev.eyakrev1.LoginResponseModel>) {
...
//                    Log.d("log", response.body().toString())
                ...
                override fun onFailure(call: Call<a103.dev.eyakrev1.LoginResponseModel>, t: Throwable) {
                    // 실패
